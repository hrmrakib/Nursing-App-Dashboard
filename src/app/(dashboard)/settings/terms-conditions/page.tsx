...
const EditTermsAndConditions = () => {
  ...
  useEffect(() => {
    if (!existing?.terms_and_conditions) return;

    // Poll until quillRef is ready (handles async init timing)
    const interval = setInterval(() => {
      if (quillRef.current) {
        quillRef.current.root.innerHTML = existing.terms_and_conditions;
        setContent(existing.terms_and_conditions);
  ...
  const handleSubmit = async () => {
    ...
    if (existing?.terms_and_conditions == content) {
