...
> = async (args, api, extraOptions) => {
  ...
  if (result.error) {
    ...
    if (status === 401) {
      ...
      showToast.error("You need to verify your email to use this feature.");
      window.location.href = "/profile";
    } else if (status === 402) {
      showToast.error("You need to upgrade your plan to use this feature.");
  ...
  if (data && data.status === false) {
    ...
    if (appStatus === 401) {
      if (!isLoggingOut && pathname !== "/login") {
        isLoggingOut = true;
        localStorage?.removeItem("access_token");
        showToast.error(data.message || "Session expired. Please login again.");
